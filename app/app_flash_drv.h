...
#if 1
#define DFU_SOFTDEVICE_END_ADDR     124*1024 //0X1f000
#else
...
#define DFU_FLASH_START_ADDR		DFU_APP1_START_ADDR            
...
//void flash_enter_power_down_enable( bool enable ); 

bool flash_addr_in_same_sector(uint32_t addr1, uint32_t addr2); 

uint32_t flash_addr_to_next_sector(uint32_t addr);

bool flash_addr_is_beginning_of_sector( uint32_t addr); 

void flash_enter_power_down_mode(void); 

void flash_release_power_down_mode(void); 

uint32_t flash_read_identfication(void);
